...
インジケーター・キーボード配列・天気・更新表示も左の時計横へ移し、中央は空けています。
バーのWS表示は非表示、既存の時計は左のメニュー横です。ワークスペースのショートカットは変わりません。右の `⋯` で独自操作を表示し、`‹` で隠します。シェル再起動後は閉じた状態で、隠しても各機能はOFFになりません。展開した中の "main monitor" はクリック操作に対応しています。
...
- バー右側の雨アイコンを左クリックすると、壁紙の雨とガラスの水滴をまとめて切り替えます。右クリックは OFF 固定です。
- OFF は雲、ON は雲から雨が降るアイコン。どちらも通常色で表示し、ホバーでも現在の状態を確認できます。
- OFF 時はプラグインをアンロードし、粒子・水滴シミュレーション・シェーダー描画を停止します。状態は次回ログイン後も保持されます。
- コマンドでも `hey-rain toggle` / `hey-rain on` / `hey-rain off` で操作できます。`hey-rain status` はバー用 JSON を返します。
